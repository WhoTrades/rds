...
<?= GridView::widget([
    ...
    'columns' => [
        ...
        [
            ...
            'value' => function ($model) {
                /** @var \whotrades\rds\models\User\User $model */
                $roleNameList = $model->getRoleNameList();

                $roleBlock = implode('', array_map(function ($item) {
                    return "<div>$item</div>";
                }, $roleNameList));

                return Html::a($roleBlock, ['/user/admin/assignments', 'id' => $model->id]);
            },
            'format' => 'raw',
        ],
        [
