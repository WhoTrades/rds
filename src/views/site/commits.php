    ...
    <tbody>
        ...
        <?foreach ($commits as $key => $commit) {?>
            ...
            <?if ($repo != $commit->jira_commit_repository) {?>
                <tr class="odd">
                    <td colspan="3">
                        <h3><a href="http://stash.finam.ru/projects/WT/repos/<?=$commit->jira_commit_repository?>/commits"><?=$commit->jira_commit_repository?></a></h3>
                    </td>
                </tr>
            ...
            <tr class="<?=$key%2 ? 'odd' : 'even'?>">
                <td>
                    <?if ($commit->jira_commit_repository) {?>
                        <a href="http://stash.finam.ru/projects/WT/repos/<?=$commit->jira_commit_repository?>/commits/<?=$commit->jira_commit_hash?>">
